...
int break_ip(char *ip){
      ...
      if ( (octet_1 < 1) && (octet_1 > 254) ){
            return -11; //1st octet is out of range
            if ( (octet_2 < 1) && (octet_2 > 254) ){
                  return -12; // 2nd octet is out of range
                  if ( (octet_3 < 1) && (octet_3 > 254) ){
                        return -13; // 3rd octet is out of range
                        if ( (octet_4 < 1) || (octet_4 > 254) ){
                              return -14; //4th octet is out of range
                        }
                  }
            }
      }
...
int check_ip(char *arg_1, char *arg_2, char *arg_3){
      ...
      if (arg_1 == NULL) {
            cout<<__FILE__<<" @"<<__LINE__<<" Missing Arg - IP/FQDN/NETWORK is required "<<std::endl; 
            clientHelp("cipc");
      }else{
      ...
      if (check_subnet == 1){
            ...
            if (is_char == 0){
                  ...
                  if (check_localhost == 1){
                        ...
                        return 7; // failing on arg: localhost
                  }
                  /* this is an fqdn */
                  is_lookup = 1;
                  int status;
                  ...
                  if ((status = getaddrinfo(arg_1, NULL, &hints, &res)) != 0) {
                        ...
                        return 2; //error on getaddrinfo lookup
                  }
                  ...
                  if (test_ip == -11){
                        ...
                        if(lookup_single == 50){
                              ...
                              cout<<std::endl;
                              return 10; // found all the ip's we were told to look for
                        }
                  }
            }
      }
